...
function AppContent({ onLogout }: { onLogout: () => void }) {
  ...
  // Uygulamanın herhangi bir yerinde bir ürün/cari adına tıklanınca, tekrar arama yapmaya
  // gerek kalmadan doğrudan o ürünün/carinin detay sayfasını açar VE o karta kaydırır
  // (sadece sekmeyi değiştirmek yetmiyor - liste uzun olduğunda kart ekranın çok altında kalabiliyor).
  const goToProduct = (productId: string) => {
    ...
    if (typeof window !== "undefined") {
      setTimeout(() => {
        const el = document.getElementById(`urun-card-${productId}`);
        if (el) {
  ...
  const goToCustomer = (customerId: string) => {
    ...
    if (typeof window !== "undefined") {
      setTimeout(() => {
        const el = document.getElementById(`cari-card-${customerId}`);
        if (el) {
          const y = el.getBoundingClientRect().top + window.scrollY - 16;
          window.scrollTo({ top: y, behavior: "smooth" });
        }
  ...
  const [deleteBatchSelectId, setDeleteBatchSelectId] = useState("");
  ...
  const deleteBatchName = async (batchId: string) => {
    ...
    // 1. onay
    if (!window.confirm(`"${batchName}" partisini silmek istediğine emin misin?\n\nBu işlem geri alınamaz (ama bir yedek kaydı tutulacak).`)) return;

    // 2. onay - parti adını yazarak teyit
    const typed = window.prompt(`Silmeyi onaylamak için parti adını tam olarak yaz:\n\n"${batchName}"`);
    if (typed !== batchName) {
      if (typed !== null) setMessage("Yazdığın ad eşleşmedi, parti silinmedi.");
      return;
    }

    // Silmeden önce bu partiyle ilgili her şeyin bir yedeğini al (nolur nolmaz geri getirebilelim)
    const batchRow = batches.find((b) => b.id === batchId);
    const backupData = {
      batch: batchRow,
      batch_costs: batchCosts.filter((c) => c.batch_id === batchId),
      batch_photos: batchPhotos.filter((p) => p.batch_id === batchId),
      cancelled_sales: sales.filter((s) => s.batch_id === batchId && s.cancelled),
      supplier_returns: supplierReturns.filter((r) => r.batch_id === batchId),
    };
    const { error: backupErr } = await supabase.from("deleted_batch_backups").insert({
      batch_id: batchId,
    ...
    if (backupErr) {
      window.alert(`Yedek alınamadığı için parti silinmedi (güvenlik amacıyla):\n${backupErr.message || backupErr}`);
      return;
    }

    const { error } = await supabase.from("batches").delete().eq("id", batchId);
    if (error) return showError(error);
    await logAction("Parti silindi", "batches", batchName, { yedek_alindi: true });
    setMessage(`"${batchName}" partisi silindi. Yedeği alındı, gerekirse geri getirebiliriz.`);
    loadAll();
  };
  ...
  const submitOdeme = async () => {
    ...
    try {
      ...
      const islemAdi = odemeTip === "toptanci" ? "Toptancıya ödeme yapıldı" : odemeTip === "kargo" ? "Kargo ödemesi yapıldı" : odemeTip === "kar_payi" ? "Kâr payı ödendi (dönem kapanışı harici, elden ödeme)" : "Diğer masraf ödendi";
      await logAction(
        islemAdi,
        "odemeler",
        odemeTip === "toptanci" ? `${supplierMap.get(odemeSupplierId)?.name || ""} — ${batchName}` : odemeTip === "kar_payi" ? odemeKarPayiAlici : batchName,
        odemeTip === "kar_payi"
          ? { tutar, kaynak: kaynakOzet, odeme_sira_no: odemeSiraNo, satici: odemeKarPayiAlici, aciklama: "Bu ödeme dönem kapanışı dışında, elden yapılan bir kâr payı ödemesidir." }
  ...
  const convertToSales = async () => {
    ...
    const rows: Record<string, unknown>[] = [];
    const convertLogBatches: string[] = []; // audit log için: hangi parti(ler)den, hangi varyanttan, kaç adet düştü
    for (const bi of depoBatchItems) {
      ...
      convertLogBatches.push(`${batchMap.get(bi.batch_id)?.name || bi.batch_id} (${bi.variant === "cep_boy" ? "Cep Boy" : "Asıl"}, ${take} adet)`);
      remainingQty -= take;
    }
    if (remainingQty > 0) return setMessage("Parti stokları yetersiz.");
    ...
    await logAction("Ön sipariş satır satışa dönüştürüldü", "preorders", customerMap.get(po.customer_id)?.name || "", {
      ...
      adet: item.qty,
      birim_fiyat: price,
      toplam_tutar: saleTotalTarget,
      partiler: convertLogBatches.join(", "),
      odeme_yontemi: paymentMethod || "cari borç",
      on_odeme_kullanildi: advanceApplied,
      kalan_tutar: remainder,
    });
                      ...
                      {productSales.map((sale) => (
                        <tr key={sale.id} style={{borderTop:"1px solid #f1f5f9"}}>
                          ...
                          <td style={{padding:"10px 16px"}}>
                            <button type="button" onClick={() => { setSalesModalProductId(null); goToCustomer(sale.customer_id); }} style={{background:"none", border:"none", padding:0, color:"#2563eb", fontWeight:600, cursor:"pointer", textDecoration:"underline"}}>
      ...
      <section className="p-3 lg:ml-72 lg:p-8">
            ...
            <div className="product-page">
              ...
              <div className="product-list">
                {displayedProducts.length ? displayedProducts.map((p) => {
                  ...
                  return (
                    <div key={p.id} id={`urun-card-${p.id}`} className={`product-card ${isOpen ? "product-card--open" : ""}`}>
            ...
            <div>
              ...
              {groups.map((g) => {
                const groupProducts = sortedProducts
                  ...
                  .map((p) => {
                    const anaItems = batchItemsForProduct(p.id).filter((i) => (i.variant || "ana") === "ana");
                    const cepItems = batchItemsForProduct(p.id).filter((i) => i.variant === "cep_boy");
                    const anaStock = anaItems.reduce((s, i) => s + Math.max(i.bought - getBatchSoldQtyForItem(i), 0), 0);
                    const cepStock = cepItems.reduce((s, i) => s + Math.max(i.bought - getBatchSoldQtyForItem(i), 0), 0);
                    const anaPriced = anaItems.filter((i) => Number(i.sale_price) > 0).sort((a, b) => new Date(b.created_at||0).getTime() - new Date(a.created_at||0).getTime());
                    const cepPriced = cepItems.filter((i) => Number(i.sale_price) > 0).sort((a, b) => new Date(b.created_at||0).getTime() - new Date(a.created_at||0).getTime());
                    const anaPrice = anaPriced.length ? Number(anaPriced[0].sale_price) : Number(p.manual_price || 0);
                    const cepPrice = cepPriced.length ? Number(cepPriced[0].sale_price) : 0;
                    return { product: p, anaStock, cepStock, anaPrice, hasCep: cepItems.length > 0, cepPrice, hasStock: anaStock > 0 || cepStock > 0 };
                  })
                        ...
                        <div key={p.id} style={{display:"flex", flexDirection:"column", alignItems:"center", gap:4}}>
                          <div style={{position:"relative", width:"100%", aspectRatio:"1/1", borderRadius:10, overflow:"hidden", background:"#f8fafc", border:"1px solid #e2e8f0", cursor:"pointer"}}
                            ...
                            <div style={{position:"absolute", top:5, right:5, display:"flex", flexDirection:"column", gap:2, alignItems:"flex-end"}}>
                              <div style={{background: anaStock > 0 ? "#16a34a" : "#dc2626", color:"#ffffff", fontSize:"0.5rem", fontWeight:700, padding:"2px 6px", borderRadius:4, whiteSpace:"nowrap"}}>
                                Büyük Boy · {anaStock > 0 ? "Stokta" : "Tükendi"} · {anaPrice ? Math.round(anaPrice).toLocaleString("tr-TR") : "-"}
                              </div>
                              {hasCep && (
                                <div style={{background: cepStock > 0 ? "#16a34a" : "#dc2626", color:"#ffffff", fontSize:"0.5rem", fontWeight:700, padding:"2px 6px", borderRadius:4, whiteSpace:"nowrap"}}>
                                  Çanta Boy · {cepStock > 0 ? "Stokta" : "Tükendi"} · {cepPrice ? Math.round(cepPrice).toLocaleString("tr-TR") : "-"}
                                </div>
                              )}
                            </div>
                          </div>
          ...
          <div className="space-y-4">
            <div style={{ display: "flex", gap: 6, marginBottom: 8, paddingTop: 4, paddingBottom: 8 }}>
              {([["giris","Yeni Parti / Ürün Girişi"],["maliyet","Maliyet Kaydı"],["rapor","Stok Raporu"]] as const).map(([key,label]) => (
                <button
                  ...
                  style={{
                    border: partiTab === key ? "none" : "1.5px solid #cbd5e1",
                    background: partiTab === key ? "#0f172a" : "#ffffff",
                    padding: "9px 16px",
                    fontSize: "0.85rem",
                    borderRadius: 10,
                    color: partiTab === key ? "#ffffff" : "#334155",
                    fontWeight: 700,
                    cursor: "pointer",
                  }}
                >
            ...
            <Card title="Parti Bazlı Ürün Girişi">
              ...
              <div className="mb-5 flex items-center gap-2">
                <select className="input flex-1" value={deleteBatchSelectId} onChange={(e) => setDeleteBatchSelectId(e.target.value)}>
                  <option value="">Silinecek parti seçin</option>
                  {sortedBatches.map((batch) => <option key={batch.id} value={batch.id}>{batch.name}</option>)}
                </select>
                <button
                  type="button"
                  className="btn-danger"
                  disabled={!deleteBatchSelectId}
                  onClick={async () => { await deleteBatchName(deleteBatchSelectId); setDeleteBatchSelectId(""); }}
                >
                  Parti Sil
                </button>
              </div>
              ...
              {batchReportFilter && (() => {
                ...
                return (
                  <div className="rounded-xl bg-slate-100 flex divide-x divide-slate-300 flex-shrink-0">
                    ...
                    {batchReportFilter !== "Tümü" && (
                      <button
                        type="button"
                        onClick={() => setPhotoManagerBatchId(batchReportFilter)}
                        className="px-3 py-2 text-center"
                        style={{ background: "none", border: "none", cursor: "pointer" }}
                      >
                        <div className="text-xs font-semibold mb-1" style={{color: photoCount > 0 ? "#2563eb" : "#64748b"}}>Parti<br/>Fotoğrafı</div>
                        <div className="text-lg font-bold" style={{color: photoCount > 0 ? "#2563eb" : "#64748b"}}>📷{photoCount > 0 ? ` ${photoCount}` : ""}</div>
                      </button>
                    )}
                  </div>
                );
                          ...
                          return (
                            <tr key={rowKey} className="border-t">
                              ...
                              <td className="p-3 font-medium" style={{ position: "sticky", left: 0, zIndex: 1, maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", backgroundColor: "#ffffff" }}>
                                <button type="button" onClick={() => goToProduct(g.product_id)} style={{background:"none", border:"none", padding:0, color:"#2563eb", fontWeight:600, cursor:"pointer", textDecoration:"underline", fontSize:"inherit", textAlign:"left"}}>
                                  {p?.name || "-"}
                                </button>
                              </td>
              ...
              return (
                ...
                >
                  ...
                  {lightboxPhotoIndex !== null && photos[lightboxPhotoIndex] && (
                    <div
                      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 200001, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
                              ...
                              {customerSales.length ? customerSales.map((sale) => {
                                return (
                                  <div key={sale.id} className="cari-sales-row">
                                    ...
                                    <div className="product-batch-cell product-batch-cell--name" style={{fontSize:"0.68rem"}}>
                                      <button type="button" onClick={() => goToProduct(sale.product_id)} style={{background:"none", border:"none", padding:0, color:"#2563eb", fontWeight:600, cursor:"pointer", textDecoration:"underline", fontSize:"inherit"}}>
                          ...
                          <ul className="mt-2 space-y-1">
                            {items.map((item) => (
                              <li key={item.id} className="flex items-center gap-2 text-sm text-slate-700">
                                <span>• <button type="button" onClick={() => goToProduct(item.product_id)} style={{background:"none", border:"none", padding:0, color:"#2563eb", fontWeight:600, cursor:"pointer", textDecoration:"underline", fontSize:"inherit"}}>{productMap.get(item.product_id)?.name || "—"}</button>{item.variant === "cep_boy" && <strong style={{color:"#dc2626"}}> (Cep Boy)</strong>} — {item.qty} adet</span>
        ...
        {showStokDetay && (() => {
          ...
          for (const bi of batchItems) {
            ...
            if (!row) {
              row = { productId: product.id, urun: product.name, tur: product.gender_category || "-", asilKalan: 0, asilDeger: 0, cepKalan: 0, cepDeger: 0 };
              byProduct.set(product.id, row);
            }
            if (bi.variant === "cep_boy") {
              row.cepKalan += kalan;
              row.cepDeger += kalan * bi.buy_price;
            } else {
              row.asilKalan += kalan;
              row.asilDeger += kalan * bi.buy_price;
            }
          }
          ...
          const sorted = [...rows].sort((a, b) => {
            ...
            if (stokSort.col === "asil_kalan") return (a.asilKalan - b.asilKalan) * dir;
            if (stokSort.col === "cep_kalan") return (a.cepKalan - b.cepKalan) * dir;
            return ((a.asilKalan + a.cepKalan) - (b.asilKalan + b.cepKalan)) * dir;
          });
              ...
              <div style={{background:"white",borderRadius:16,width:"100%",maxWidth:900,maxHeight:"90vh",display:"flex",flexDirection:"column",overflow:"hidden"}} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={{overflow:"auto",padding:"0 24px 24px",flex:1}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:"0.8rem"}}>
                    <thead>
                      <tr>
                        <th style={{padding:"8px 10px",textAlign:"left",fontWeight:600,color:"#64748b",position:"sticky",top:0,backgroundColor:"#f8fafc",zIndex:2,height:37}} onClick={() => setStokSort((p) => ({col:"urun", dir: p.col === "urun" && p.dir === "asc" ? "desc" : "asc"}))}>Ürün Adı{stokSort.col === "urun" ? (stokSort.dir === "asc" ? " ↑" : " ↓") : " ↕"}</th>
                        <th style={{padding:"8px 10px",textAlign:"left",fontWeight:600,color:"#64748b",position:"sticky",top:0,backgroundColor:"#f8fafc",zIndex:2,height:37}} onClick={() => setStokSort((p) => ({col:"tur", dir: p.col === "tur" && p.dir === "asc" ? "desc" : "asc"}))}>Tür{stokSort.col === "tur" ? (stokSort.dir === "asc" ? " ↑" : " ↓") : " ↕"}</th>
                        <th style={{padding:"8px 10px",textAlign:"right",fontWeight:700,color:"#1e3a8a",position:"sticky",top:0,backgroundColor:"#dbeafe",zIndex:2,height:37}} onClick={() => setStokSort((p) => ({col:"asil_kalan", dir: p.col === "asil_kalan" && p.dir === "asc" ? "desc" : "asc"}))}>Asıl Kalan{stokSort.col === "asil_kalan" ? (stokSort.dir === "asc" ? " ↑" : " ↓") : " ↕"}</th>
                        <th style={{padding:"8px 10px",textAlign:"right",fontWeight:600,color:"#1e3a8a",position:"sticky",top:0,backgroundColor:"#dbeafe",zIndex:2,height:37}}>Asıl Değer</th>
                        <th style={{padding:"8px 10px",textAlign:"right",fontWeight:700,color:"#334155",position:"sticky",top:0,backgroundColor:"#f1f5f9",zIndex:2,height:37}} onClick={() => setStokSort((p) => ({col:"cep_kalan", dir: p.col === "cep_kalan" && p.dir === "asc" ? "desc" : "asc"}))}>Cep Kalan{stokSort.col === "cep_kalan" ? (stokSort.dir === "asc" ? " ↑" : " ↓") : " ↕"}</th>
                        <th style={{padding:"8px 10px",textAlign:"right",fontWeight:600,color:"#334155",position:"sticky",top:0,backgroundColor:"#f1f5f9",zIndex:2,height:37}}>Cep Değer</th>
                      </tr>
                      <tr>
                    ...
                    <tbody>
                      {sorted.map((r) => (
                        <tr key={r.productId} style={{borderBottom:"1px solid #f1f5f9"}}>
                          <td style={{padding:"7px 10px",fontWeight:500}}>
                            <button type="button" onClick={() => { setShowStokDetay(false); goToProduct(r.productId); }} style={{background:"none", border:"none", padding:0, color:"#2563eb", fontWeight:600, cursor:"pointer", textDecoration:"underline", fontSize:"inherit"}}>{r.urun}</button>
                          </td>
                          <td style={{padding:"7px 10px",color:"#64748b"}}>{r.tur}</td>
                          <td style={{padding:"7px 10px",textAlign:"right",fontWeight:600}}>{r.asilKalan > 0 ? r.asilKalan : "-"}</td>
                          <td style={{padding:"7px 10px",textAlign:"right",color:"#64748b"}}>{r.asilKalan > 0 ? money(r.asilDeger) : "-"}</td>
                          <td style={{padding:"7px 10px",textAlign:"right",fontWeight:600}}>{r.cepKalan > 0 ? r.cepKalan : "-"}</td>
                          <td style={{padding:"7px 10px",textAlign:"right",color:"#64748b"}}>{r.cepKalan > 0 ? money(r.cepDeger) : "-"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <Table
                ...
                rows={sortedSales.map((sale) => {
                  ...
                  return [
                    ...
                    <button key="cust" type="button" onClick={() => goToCustomer(sale.customer_id)} style={{background:"none", border:"none", padding:0, color:"#2563eb", fontWeight:600, cursor:"pointer", textDecoration:"underline", fontSize:"inherit"}}>{customerMap.get(sale.customer_id)?.name || "-"}</button>,
                    <button key="prod" type="button" onClick={() => goToProduct(sale.product_id)} style={{background:"none", border:"none", padding:0, color:"#2563eb", fontWeight:600, cursor:"pointer", textDecoration:"underline", fontSize:"inherit"}}>{saleProductName(sale)}</button>,
